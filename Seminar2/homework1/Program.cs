...
Console.WriteLine("---------");
Console.Write("Введите число: "); // есть вариант с реализацией большего количества символов
...
for (i = 0; i < L1; i++)
{
    if (i == 1) {Console.WriteLine($"Второй символ: {A_num1[i]}");}
}
...
for (i=0; i<L2; i++)
{
    if (i == 1) {Console.WriteLine($"Второй символ: {A_num2[i]}");}
}
...
for (i=0; i<L3; i++)
{
    if (i == 1) {Console.WriteLine($"Второй символ: {A_num3[i]}");}
}